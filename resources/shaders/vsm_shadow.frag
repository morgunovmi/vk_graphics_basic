...
const float g_minVariance = 0.0001f;

float vsm_filter(vec2 moments, float fragDepth)
{
  float p = float(fragDepth <= moments.x);
...
void main()
{
  ...
  const float shadow  = outOfView ? 1.0f : vsm_filter(textureLod(shadowMap, shadowTexCoord, 0).xy, posLightSpaceNDC.z);
